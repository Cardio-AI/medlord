...
class ControlNetConditioningEmbedding(nn.Module):
    def __init__(
        ...
        r = 1.0
        num_channels = [max(int(r * ch), 1) for ch in num_channels]  # Apply scaling

        self.conv_in = Convolution(
            spatial_dims=spatial_dims,
            in_channels=in_channels,  # Matches input conditioning data
            out_channels=num_channels[0],  # Scaled ControlNet channels
            strides=2, #changed from stride=1 for memory reduction.
            kernel_size=3,
            padding=1,
            conv_only=True,
        )
        ...
        self.conv_out = zero_module(
            Convolution(
                ...
                out_channels=out_channels,  # Matches first ControlNet block input
                strides=1,
                kernel_size=3,
                padding=1,
                conv_only=True,
...
class ControlNet(nn.Module):
    ...
    def __init__(
        ...
        # Scale factor for reducing the channel size, using r=0.25 from ControlnetXS, implement this to be variable later:
        r = 1.0
        norm_num_groups = int(r* norm_num_groups)
        controlnet_num_channels = [max(int(r * ch), 1) for ch in num_channels]
        self.block_out_channels = controlnet_num_channels
        ...
        self.conv_in = Convolution(
            ...
            out_channels=controlnet_num_channels[0],
            strides=1,
            kernel_size=3,
            padding=1,
            conv_only=True,
        ...
        # time
        time_embed_dim = controlnet_num_channels[0] * 4
        self.time_embed = nn.Sequential(
            nn.Linear(controlnet_num_channels[0], time_embed_dim),
            nn.SiLU(),
        ...
        output_channel = controlnet_num_channels[0]  # Scaled ControlNet channels
        input_channel = output_channel

        controlnet_block = Convolution(
            spatial_dims=spatial_dims,
            in_channels=input_channel,   # Scaled input channels
            out_channels=int(output_channel/r), # Scaled output channels
            strides=1,
            kernel_size=1,
            padding=0,
            conv_only=True,
        ...
        for i in range(len(controlnet_num_channels)):
            input_channel = output_channel
            output_channel = controlnet_num_channels[i]
            

            is_final_block = i == len(controlnet_num_channels) - 1
            ...
            for _ in range(num_res_blocks[i]):
                controlnet_block = Convolution(
                    spatial_dims=spatial_dims,
                    in_channels=output_channel,   # Scaled input channels
                    out_channels=int(output_channel/r), # Scaled output channels
                    strides=1,
                    kernel_size=1,
                    padding=0,
        ...
        # mid
        mid_block_channel = controlnet_num_channels[-1]
        ...
        controlnet_block = Convolution(
            spatial_dims=spatial_dims,
            in_channels=mid_block_channel,
            out_channels=int(mid_block_channel/r),
            strides=1,
            kernel_size=1,
            padding=0,
            conv_only=True,
