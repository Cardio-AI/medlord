...
class EMAQuantizer(nn.Module):
    ...
    def quantize(self, inputs: torch.Tensor, batch_size: int = 32) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        ...
        Args:
            inputs: Encoding space tensors
            batch_size: Size of the batch to use for quantization. Defaults to 32.
        ...
        encoding_indices = torch.cat(quantized_indices_batches)
       # encodings = torch.nn.functional.one_hot(encoding_indices, self.num_embeddings).float()
    ...
    def forward(self, inputs: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        ...
        ####Maybe put one_hot encoding into self.training condition, to prevent it beeing used during sampling.
