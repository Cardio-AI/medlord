...
class ControlNetConditioningEmbedding(nn.Module):
    ...
    def __init__(
        ...
        for i in range(len(num_channels) - 1): #changed from -1
    ...
    def forward(self, conditioning):
        if conditioning.dtype != torch.float32: #added to handle categorical masks
...
class MultiControlNet(nn.Module):
    ...
    def __init__(
        ...
        #conditioning_embedding_in_channels1: int = 1,
        #conditioning_embedding_in_channels2: int = 1,
        ...
        ### Changed for Multi Conditioning
        '''    
        self.controlnet_cond_embedding1 = ControlNetConditioningEmbedding(
            spatial_dims=spatial_dims,
            in_channels=conditioning_embedding_in_channels1,
        ...
        self.controlnet_cond_embedding2 = ControlNetConditioningEmbedding(
            spatial_dims=spatial_dims,
            in_channels=conditioning_embedding_in_channels2,
            num_channels=conditioning_embedding_num_channels,
            out_channels=num_channels[0],
        )'''
    ...
    def forward(
        ...
        '''
        controlnet_cond1 = self.controlnet_cond_embedding1(controlnet_cond_list[0])
                #### Added interpolation to assure same tensor sizes. Try best interpolation mode.
        
        if controlnet_cond1.shape[2:] != h.shape[2:]:
            controlnet_cond1 = F.interpolate(controlnet_cond1, h.shape[2:],mode="trilinear")
        controlnet_cond2 = self.controlnet_cond_embedding2(controlnet_cond_list[1])

                #### Added interpolation to assure same tensor sizes. Try best interpolation mode.
        
        if controlnet_cond2.shape[2:] != h.shape[2:]:
            controlnet_cond2 = F.interpolate(controlnet_cond2, h.shape[2:],mode="trilinear")'''


        controlnet_cond_embedding = self.controlnet_cond_embedding  # Assuming you have only one embedding layer
        controlnet_cond_vars = []  # List to store dynamically created variables

        for i, controlnet_cond_input in enumerate(controlnet_cond_list):
            controlnet_cond = controlnet_cond_embedding(controlnet_cond_input)
            
            # Added interpolation to assure same tensor sizes. Try best interpolation mode.
            if controlnet_cond.shape[2:] != h.shape[2:]:
                controlnet_cond = F.interpolate(controlnet_cond, h.shape[2:], mode="trilinear")

            # Append the dynamically generated variable to the list
            controlnet_cond_vars.append(controlnet_cond)

        # Now you have controlnet_cond1, controlnet_cond2, ..., controlnet_condN stored in controlnet_cond_vars
        # You can access them using list indexing, e.g., controlnet_cond_vars[0], controlnet_cond_vars[1], etc.
        multicond = torch.sum(torch.stack(controlnet_cond_vars), dim=0)


        
        
        #multicond = controlnet_cond1 + controlnet_cond2      
        
        largest_value = multicond.max()
        if largest_value > 1:
            multicond = multicond / largest_value
