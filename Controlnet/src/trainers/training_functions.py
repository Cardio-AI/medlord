...
def train_epoch_controlnet(
    ...
    for step, x in pbar:
        images = x["image"]["data"].to(device)
        reports = x["report"].to(device)
        cond = x["mask"]["data"].to(device,dtype=torch.float32)
        ...
        with autocast(enabled=True):
            ...
            if scheduler.prediction_type == "v_prediction":
                ...
                target = noise
            loss = F.l1_loss(noise_pred.float(), target.float()) #i replaced mse with l1 loss
        ...
        scaler.update()
        #optimizer.step() #added this for testing purpose
