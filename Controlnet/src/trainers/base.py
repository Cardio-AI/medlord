...
class BaseTrainer:
    def __init__(self, args):
        ...
        if args.vqvae_checkpoint:
            ...
            self.vqvae_model.load_state_dict(vqvae_checkpoint["model_state_dict"]) #changed model_state_dict
        ...
        elif args.model_type == "crossattn":
            self.model = DiffusionModelUNet(
                spatial_dims=args.spatial_dimension,
                in_channels=ddpm_channels,
                out_channels=ddpm_channels,
        ...
        if self.beta_schedule == "cosine":
            self.scheduler = DDPMScheduler( #was DDPM before
                num_train_timesteps=1000,#1000 before
        ...
        self.optimizer = torch.optim.Adam(params=self.model.parameters(), lr=2.5e-5)#changed from 2.5e-5
        if checkpoint_path.exists():
